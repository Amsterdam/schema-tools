...
def test_model_mocker_factory_sets_model_mocker_dataset_and_table_schema_through_dynamic_model(
    ...
    """Prove that the model_mocker_factory creates a DynamicModelMocker that is linked to a DynamicModel for a Dataset with a DatasetTableSchema"""
    table_schema = afval_dataset.schema.tables[0]  # afvalwegingen.containers

    model_mocker_cls = model_mocker_factory(
        afval_dataset, table_schema, base_app_name="dso_api.dynamic_api"
    )
    # model_mocker_cls._meta is the DjangoModelFactory.Meta, so get_model_class() gives the Django Model for which
    ...
    assert type(AfvalwegingenContainersModel.table_schema()) == DatasetTableSchema
    assert AfvalwegingenContainersModel.table_schema() == table_schema
    assert (
...
def test_schema_model_mockers_factory(afval_dataset):
    """Prove that schema_model_mockers_factory works for an AmsSchema with 2 tables, thus, 2 DynamicModel instances"""
    model_mockers = {
